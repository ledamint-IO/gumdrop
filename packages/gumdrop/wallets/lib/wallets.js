"use strict";
var __rest = (this && this.__rest) || function (s, e) {
    var t = {};
    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
        t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) {
            if (e.indexOf(p[i]) < 0 && Object.prototype.propertyIsEnumerable.call(s, p[i]))
                t[p[i]] = s[p[i]];
        }
    return t;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getSolongWallet = void 0;

const wallet_adapter_sollet_1 = require("@j0nnyboi/wallet-adapter-safecoin");
var WalletName;
(function (WalletName) {
    WalletName["Sollet"] = "Sollet";
    // WalletConnect = 'WalletConnect',
})(WalletName = exports.WalletName || (exports.WalletName = {}));

const getSolletWallet = (_a = {}) => {
    var { provider } = _a, config = __rest(_a, ["provider"]);
    return ({
        name: WalletName.Sollet,
        url: 'https://wallet.safecoin.org',
        icon: 'data:image/svg+xml;base64,',
        adapter: () => new wallet_adapter_sollet_1.SolletWalletAdapter(Object.assign({ provider: 'https://wallet.safecoin.org' }, config)),
    });
};
exports.getSolletWallet = getSolletWallet;
